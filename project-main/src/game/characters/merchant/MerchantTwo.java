 ...
 */
public class MerchantTwo extends Actor implements MerchantNPC {
     ...
     */
    public MerchantTwo() {
        super("Thiollier the Belarut Sewer Merchant", '$', 0);
        ShortSword quickStepSS = new ShortSword();
        GreatKnife quickStepGK = new GreatKnife();
        quickStepSS.addCapability(QUICKSTEP);
        quickStepGK.addCapability(QUICKSTEP);
        this.inventory.add ( new ShadowtreeFragment() );
        this.inventory.add( quickStepSS );
        this.inventory.add( quickStepGK );
        this.inventory.add( new FlaskOfRejuvenation() );
    }
    ...
    @Override
    public String greet() {
        return this.name +": 'Greetings, Tarnished. Behold my weapon and the arts it wields for your perusal.' ";
    }
    ...
    public Boolean outofsale() {
        ...
        if ( this.inventory.isEmpty() ){
            System.out.println( this.name + ": 'It appears I have naught left to offer you.' " );
            return true;
        }
        return false;
    }
